...
**Uno strumento desktop per il monitoraggio della rete domestica su Windows — grafici del traffico in tempo reale, scoperta dei dispositivi e avvisi basati su regole.**

[![Licenza](https://img.shields.io/badge/Licenza-MIT-blue.svg)](LICENSE)
[![Python](https://img.shields.io/badge/Python-3.11+-green.svg)](https://www.python.org/downloads/)
[![Piattaforma](https://img.shields.io/badge/Piattaforma-Windows%2010%2F11-blue.svg)](#requisiti-di-sistema)
...
HomeNetMonitor è un'applicazione desktop PyQt6 che osserva in tempo reale il
traffico della rete domestica, scopre i dispositivi collegati e genera avvisi
quando qualcosa sembra sospetto — un nuovo dispositivo che si collega, un
picco di banda, traffico verso un IP specifico o attività su una porta
specifica. Tutti i dati — connessioni, dispositivi, alert e cache di
geolocalizzazione — restano in locale in un database SQLite; nulla viene
inviato all'esterno tranne le richieste di geolocalizzazione IP verso il
servizio gratuito [ip-api.com](http://ip-api.com).
...
| 🔗 **Connessioni attive** | Tabella in tempo reale di tutti i flussi di rete catturati con codifica colori (verde = sicuro, giallo = sconosciuto, rosso = segnalato) |
| 🔔 **Regole di alert** | Avvisi personalizzati su: nuovo dispositivo, soglia di banda, comunicazione verso un IP specifico, attività su una porta |
...
- **[Npcap](https://npcap.com/#download)**: libreria per la cattura di pacchetti su Windows — **obbligatoria** per il packet sniffer e la scansione ARP. Da installare prima di avviare l'applicazione, selezionando l'opzione *"Install Npcap in WinPcap API-compatible Mode"*.
...
| `scapy` | >= 2.5.0 | Cattura pacchetti e scansione ARP |
| `psutil` | >= 5.9.8 | Info sulle interfacce di rete |
| `netifaces2` | >= 0.0.1 | Rilevamento gateway e subnet |
| `requests` | >= 2.31.0 | Chiamate API di geolocalizzazione |
| `pyinstaller` | >= 6.4.0 | Creazione dell'eseguibile standalone |
...
#### Top 5 destinazioni
Tabella con le 5 destinazioni che hanno ricevuto più byte nella sessione corrente, con hostname, paese (con bandiera emoji) e volume di dati.
...
- `DnsResolver` — cache, timeout, lookup asincroni, eviction
- `PacketSniffer` — parsing pacchetti TCP/UDP, conteggio byte
...
│       ├── formatting.py       <- Utilità di formattazione condivise (format_bytes)
...
- ogni **ora** viene eseguita la pulizia dei dati più vecchi del periodo di retention
...
Assicurati di averla avviata come Amministratore e che Npcap sia installato. Controlla la barra di stato: se mostra `Admin: X` la scansione ARP è disabilitata.

**Il grafico del traffico è vuoto.**
...
Vai nella scheda **Devices**, fai clic destro sul dispositivo e scegli **Block Device (Firewall)**. L'applicazione aggiungerà una regola al Windows Firewall che blocca il traffico in uscita verso quell'IP. Per rimuovere la regola, apri Windows Defender Firewall e cerca le regole con prefisso `HomeNetMonitor_Block_`.
